...
$isPo = $ok && (($claims['kind'] ?? '') === 'po');
$isCancelled = $ok && !empty($live['cancelled']);
$periodLabel = '';
if ($ok && !$isPo) {
    $periodLabel = date('d F Y', strtotime((string) $claims['from_date']))
        . ' — ' . date('d F Y', strtotime((string) $claims['to_date']));
}
$poNo = (string) ($live['po_no'] ?? '');
$dateMeans = (!$isPo && (($claims['date_field'] ?? '') === 'po')) ? 'PO date' : 'Uploaded date';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>PO document pack verification — <?= htmlspecialchars($companyName) ?></title>
    <style>
...
<div class="wrap">
    <div class="card">
        <div class="head">
            ...
            <p>Purchase-order document pack verification · التحقق من مستندات أمر الشراء</p>
        </div>
        <div class="body">
            <?php if (!$ok): ?>
            ...
            <div class="banner match">
                ...
                <span class="sub">This pack matches the company’s live supplier invoice and TT copies.</span>
            </div>
            <?php elseif ($isCancelled): ?>
            <div class="banner changed">
                Signed pack — purchase order cancelled
                <span class="sub">The QR is genuine, but this purchase order is now cancelled in the company’s records.</span>
            </div>
            <?php else: ?>
            <div class="banner changed">
                Signed pack — files have changed
                <span class="sub">The QR is genuine, but live documents now differ from what was printed (for example a later upload, deletion, or cancellation).</span>
            </div>
            <?php endif; ?>
            ...
            <div class="meta">
                <?php if ($isPo): ?>
                <div class="chip"><span class="k">Purchase order</span><span class="v"><?= htmlspecialchars($poNo !== '' ? $poNo : '—') ?></span></div>
                <div class="chip"><span class="k">PO date</span><span class="v"><?= !empty($live['po_date']) ? date('d M Y', strtotime((string) $live['po_date'])) : '—' ?></span></div>
                <?php else: ?>
                <div class="chip"><span class="k">Period</span><span class="v"><?= htmlspecialchars($periodLabel) ?></span></div>
                <div class="chip"><span class="k">Date means</span><span class="v"><?= htmlspecialchars($dateMeans) ?></span></div>
                <?php endif; ?>
                ...
                <div class="chip">
                    <span class="k">On printed pack</span>
                    <span class="v"><?= (int) $claims['invoice_count'] ?> invoices · <?= (int) $claims['tt_count'] ?> TT
                        <?php if (!$isPo): ?> · <?= (int) $claims['po_count'] ?> POs<?php endif; ?></span>
                </div>
                <div class="chip">
                    <span class="k">Live records now</span>
                    <span class="v"><?= (int) ($live['invoice_count'] ?? 0) ?> invoices · <?= (int) ($live['tt_count'] ?? 0) ?> TT
                        <?php if (!$isPo): ?> · <?= (int) ($live['po_count'] ?? 0) ?> POs<?php endif; ?></span>
                </div>
            </div>

            <?php if (!empty($live['rows'])): ?>
            <table>
                <thead>
                    <tr>
                ...
                <tbody>
                <?php foreach ($live['rows'] as $i => $row): ?>
                    <tr>
                        <td><?= $i + 1 ?></td>
                        <td><?= htmlspecialchars((string) ($row['po_no'] ?? '')) ?></td>
                        <td><?= !empty($row['po_date']) ? date('d M Y', strtotime((string) $row['po_date'])) : '—' ?></td>
                        <td class="num"><?= (int) ($row['invoice_count'] ?? 0) ?></td>
                        <td class="num"><?= (int) ($row['tt_count'] ?? 0) ?></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
            ...
            <p class="foot">
                Bank document pack · مجموعة مستندات البنك<br>
                Supplier names are not shown on this public page.
            </p>
        </div>
    </div>
</div>
