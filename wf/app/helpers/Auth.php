...
class Auth {
    ...
    public static function startSession(): void {
        if (session_status() === PHP_SESSION_NONE) {
            ...
            session_set_cookie_params([
                ...
                'path'     => '/',
                'secure'   => true,   // AUDIT FIX S4: must be true — app runs on HTTPS
                'httponly' => true,
                'samesite' => 'Strict'
            ]);
            session_start();
    ...
    public static function login(string $email, string $password): bool|string {
        ...
        if ($recentAttempts && (int)$recentAttempts['cnt'] >= 5) {
        ...
        if (!$user) {
            $db->insert("INSERT INTO login_attempts (ip_address, email) VALUES (?,?)", [$ip, $email]);
    ...
    public static function warehouseName(): string {
        return $_SESSION['warehouse_name'] ?? 'No Warehouse';
    }

    // Set warehouse in session
    public static function setWarehouse(int $id, string $name): void {
        $_SESSION['warehouse_id']   = $id;
        $_SESSION['warehouse_name'] = $name;
    }

    // Clear warehouse from session (force re-select)
    public static function clearWarehouse(): void {
        unset($_SESSION['warehouse_id'], $_SESSION['warehouse_name']);
    }

    // Redirect to warehouse selector if no warehouse chosen
    public static function requireWarehouse(): void {
